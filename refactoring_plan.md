...
| BCR-14 | `rename_tab` | `runtime/controllers/rename_tab.zig` | `RenameTabHandler` and workspace aggregate | Ready | Commits `a35fe64`, `dc5f8ce`; owned `TabRenamed`, command/controller tests, and vertical backpressure test exist. |
...
| BRE-05 | `history_response` | `runtime/history_response_controller.zig` | History response controller | Ready | Receiver rearming, exact query-result ownership transfer/disposal, stale-generation routing, control-client close policy, response backpressure, and delivery pumping are isolated; rearm failure no longer leaks the received query result; transport coverage, `just check`, and 1702 Debug/ReleaseFast tests pass. |
...
| BRE-19 | `stopped` | `runtime/stop_signal.zig` | Infrastructure stop-signal coordinator | Ready | The optional borrowed queue owns one explicit arming/completion contract independent from client-requested graceful shutdown. Unit tests cover disabled operation, exact queue identity, scheduler failure without losing the borrow, successful terminal completion, exact source-error propagation, and real token consumption; 1827 Debug/ReleaseFast tests, three repeated transport runs per mode, and `just check` pass. |
