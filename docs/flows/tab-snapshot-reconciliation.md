...
releases copy mode, paste state, reported focus and graphics. For an active
tab, it synchronizes focus, re-offers attached pane sizes and requests an
attachment for each detached pane. A pane with an attachment request already
pending does not receive a duplicate request.
