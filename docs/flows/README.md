...
| [Pane input](pane-input.md) | Routed keyboard, streamed paste or mouse input targets a child | The client validates one active target, preserves one paste owner, applies viewport policy and delivers bounded bytes to the runtime | Model, session, encoding, effect-order, backpressure and protocol tests |
