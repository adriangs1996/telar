...
fallback of 80 by 24. Nonzero window pixels refresh the client's capability
state, and `TerminalCapabilities.cellSize` resolves the cell dimensions.

## Model transaction

`ClientModel` owns the resolved `schema.TerminalSize`. The value contains the
host grid plus the best cell-pixel dimensions currently known. The model calls
`TerminalSize.validate` before mutation, so an empty grid or one beyond the
shared frame-cell bound reaches no allocator or effect.

An exact repeated value is a no-op. An accepted value advances
`Version.host`, stores the complete geometry and updates the cell size held by
the tab collection. The tab collection applies it to every current tab and
retains it for tabs created or discovered later. A terminal pixel response
uses the same resolved transition, so capability negotiation cannot leave a
second geometry value outside the model.
...
At the client-loop boundary, `Client.observeModel` publishes `Version.host`.
The presenter compares it with the version last painted, invalidates the host
projection and folds the resize into its paced frame. A repeated measurement
...
- `src/frontend/workspace/tabs.zig` proves that current and future tabs share
  one cell geometry.
- `src/frontend/client/model.zig` proves validation, atomic commit, no-op
  behavior and the isolated host revision.
