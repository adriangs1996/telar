...
def validate(result, directory, options, warmup):
    ...
    if requested_rate is not None:
        ...
        require(load.get('rate_met') is True, f'background output missed its requested rate: {load}')
...
def main():
    ...
    parser.add_argument('--ghostty-control', action='store_true',
                        help='also measure standalone Ghostty once per case and round')
    ...
    manifest = dict(started_at=time.strftime('%Y-%m-%dT%H:%M:%S%z'), command_line=sys.argv,
                    ...
                    order='AB on even rounds, BA on odd rounds; optional Ghostty precedes even rounds and follows odd rounds',
        ...
        for round_index in range(args.rounds):
            ...
            for case in args.cases:
                ...
                for version in order:
                    ...
                    result.update(**context, binary_sha256=binary_hash)
