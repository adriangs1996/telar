...
class Fixture:
    ...
    def draw_marker(self, red_variation=False):
        red = '220;20;60' if red_variation else '230;20;60'
        rgb = red if self.color == 0 else '20;220;200'
    ...
    def await_viewport(self):
        ...
        while not (self.directory / 'viewport.ready').is_file():
            ...
            if now >= next_redraw:
                self.refresh_size()
                # Both shades remain red; changing the cell forces a fresh
                # frame through an outer terminal after the TUI has resized.
                red_variation = not red_variation
                self.draw_marker(red_variation)
                next_redraw = now + .15
            time.sleep(.02)
        self.refresh_size()
