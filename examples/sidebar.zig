...
const GenericHits = @import("telar-frontend").GenericHits;
const GenericFocus = @import("telar-frontend").GenericFocus;
...
const text_module = @import("telar-core").text;
const GenericInput = @import("telar-frontend").GenericInput;
const ResizeWatcherType = @import("telar-frontend").ResizeWatcher;
const TtyType = @import("telar-frontend").Tty;
const enter_module = @import("telar-frontend").enter;
const leave = @import("telar-frontend").leave;
const ScreenType = @import("telar-frontend").Screen;
const PacerType = @import("telar-frontend").Pacer;
const coalesce_module = @import("telar-frontend").coalesce;
const writeClipboard_module = @import("telar-frontend").writeClipboard;
...
const bg: ColorType = .{ .rgb = .{ 0x10, 0x10, 0x10 } };
const raised: ColorType = .{ .rgb = .{ 0x18, 0x18, 0x18 } };
const chip_bg: ColorType = .{ .rgb = .{ 0x33, 0x33, 0x33 } };
const white: ColorType = .{ .rgb = .{ 0xe8, 0xe8, 0xe8 } };
const fg: ColorType = .{ .rgb = .{ 0xb0, 0xb0, 0xb0 } };
const muted: ColorType = .{ .rgb = .{ 0x6c, 0x6c, 0x6c } };
const faint: ColorType = .{ .rgb = .{ 0x45, 0x45, 0x45 } };
const apricot: ColorType = .{ .rgb = .{ 0xff, 0xc7, 0x99 } };
const mint: ColorType = .{ .rgb = .{ 0x99, 0xff, 0xe4 } };
const red: ColorType = .{ .rgb = .{ 0xff, 0x80, 0x80 } };
...
pub const Hits = GenericHits(Action, 128);
...
pub const FocusReg = GenericFocus(FocusId, 64);
...
fn view(state: *State, buf: *BufferType) void {
    state.hits.clear();
...
fn drawDialog(context: DrawContext, index: usize) void {
    ...
    const inside: RectType = .{ .x = frame.x + 2, .y = frame.y + 2, .w = frame.w - 4, .h = frame.h - 4 };
    ...
    for (dialog_choices, 0..) |choice, choice_index| {
        const width = measure_module(choice) + 4;
        if (x + width > inside.x + inside.w) {
            break;
        }
        const rect: RectType = .{ .x = x, .y = y, .w = width, .h = 1 };
...
fn drawSidebar(state: *State, buf: *BufferType, area: RectType) void {
    ...
    const inside: RectType = .{ .x = area.x + 1, .y = area.y + 1, .w = area.w - 2, .h = area.h - 2 };
...
fn rule(buf: *BufferType, area: RectType, y: u16) u16 {
    const style: StyleType = .{ .fg = faint, .bg = bg };
    var x = area.x + 1;
...
fn drawSearch(context: DrawContext, y: u16) u16 {
    ...
    const row: RectType = .{ .x = area.x, .y = y, .w = area.w, .h = 1 };
    ...
    const field_bg: ColorType = if (focused) raised else bg;
    var x = row.x + 1;
...
fn drawChipAt(context: DrawContext, chip: ChipDraw) u16 {
    ...
    const buf = context.buffer;
    const area = context.area;
    const width = measure_module(chip.label) + 2;
    ...
    const fill: ColorType = if (hovered) chip_bg else bg;
...
fn drawTabs(context: DrawContext, y: u16) u16 {
    ...
    const area = context.area;
    const row: RectType = .{ .x = area.x, .y = y, .w = area.w, .h = 1 };
    const underline: RectType = .{ .x = area.x, .y = y + 1, .w = area.w, .h = 1 };
    ...
    for (state.tabs, 0..) |tab, index| {
        ...
        const color: ColorType = if (active) white else if (hovered) fg else muted;
        const start = x;
...
fn drawScope(context: DrawContext, y: u16) u16 {
    ...
    const row_bg: ColorType = if (hovered) raised else bg;
...
fn drawList(state: *State, buf: *BufferType, area: RectType) void {
    buf.fill(area, .{ .glyph = " ", .style = .{ .bg = bg } });
    ...
    const content: RectType = .{ .x = area.x, .y = area.y, .w = area.w -| 1, .h = area.h };
...
fn drawSectionHeader(context: DrawContext, y: u16, section: Section) void {
    ...
    const rule_end = row.x + row.w -| measure_module(count_text) -| 2;
...
fn drawTaskLine(context: DrawContext, position: TaskLine) void {
    ...
    const row_bg: ColorType = if (selected)
        .{ .rgb = .{ 0x1e, 0x1e, 0x1e } }
    else if (hovered)
        raised
    else
...
fn chipWidth(chip: Chip) u16 {
    return measure_module(chip.glyph()) + 1 + measure_module(chip.label()) + 2;
}
...
fn drawTaskChip(context: DrawContext, index: usize, chip: Chip) u16 {
    ...
    const fill: ColorType = if (chip.filled() or hovered) chip_bg else bg;

    const rect: RectType = .{ .x = x, .y = area.y, .w = width, .h = 1 };
...
fn drawStatus(buf: *BufferType, draw: StatusDraw) u16 {
    ...
    const color: ColorType = switch (task.status) {
    ...
    const detail: u16 = if (task.status_detail.len == 0) 0 else measure_module(task.status_detail) + 3;
    const width = prefix + measure_module(word) + detail + 1;
    if (width > area.w) {
        return 0;
    }
...
fn drawFooter(state: *State, buf: *BufferType, area: RectType) void {
    buf.fill(area, .{ .glyph = " ", .style = .{ .bg = bg } });
...
fn drawPane(state: *const State, buf: *BufferType, area: RectType) void {
    buf.fill(area, .{ .glyph = " ", .style = .{ .bg = bg } });
...
fn update(state: *State, event: EventType) void {
    switch (event) {
        .incomplete => {},
        .terminal_response => {},
...
fn handleSelection(state: *State, mouse: EventType.Mouse) bool {
    const at: PointType = .{ .x = mouse.x, .y = mouse.y };
...
fn copySelection(state: *State) void {
    ...
    const copied = text_module(source, range.expanded(source), &state.clipboard);
...
fn handleKey(state: *State, key: EventType.Key) void {
...
fn searchKey(state: *State, key: EventType.Key) void {
...
fn pasteInto(state: *State, key: EventType.Key) void {
    switch (key.code) {
...
fn dialogKey(state: *State, key: EventType.Key, index: usize) void {
    switch (key.code) {
...
fn taskKey(state: *State, key: EventType.Key, index: usize) void {
    switch (key.code) {
...
fn tabKey(state: *State, key: EventType.Key, index: usize) void {
    switch (key.code) {
...
fn chromeKey(state: *State, key: EventType.Key) void {
    switch (key.code) {
...
fn switchTab(state: *State, key: EventType.Key) void {
    if (key.code == .left) {
...
fn globalChar(state: *State, c: EventType.Char) void {
    if (c.eql("q")) {
        state.quit = true;
        return;
    }
...
fn inputActor(io: std.Io, queue: *std.Io.Queue(Message)) std.Io.Cancelable!void {
...
pub fn main(init: std.process.Init) !void {
    ...
    try w.writeAll(enter_module);
    try w.flush();
    defer {
        w.writeAll(leave) catch {};
        w.flush() catch {};
    }

    const start_size = tty.size();
    var screen = try ScreenType.init(gpa, start_size.cols, start_size.rows);
    defer screen.deinit();
    ...
    var queue: std.Io.Queue(Message) = .init(&queue_buf);
    ...
    while (!state.quit) {
        ...
        while (true) {
            const before = n;
            n = coalesce_module(Message, batch[0..n], Message.coalesceKey);
            ...
            const timestamp = std.Io.Timestamp.fromNanoseconds(@intCast(deadline)).withClock(.awake);
...
fn sendClipboard(state: *State, w: *std.Io.Writer) !void {
    ...
    writeClipboard_module(w, payload) catch |err| switch (err) {
...
test "everything the sidebar draws as clickable is registered" {
    ...
    var buf = try BufferType.init(gpa, 100, 40);
    defer buf.deinit();
    ...
    try std.testing.expect(seen_tab and seen_task and seen_chip and seen_hint);
    try std.testing.expect(seen_scope and seen_new and seen_palette and seen_search);
}
...
test "an action chip takes the click, not the task row beneath it" {
    ...
    try std.testing.expectEqual(@as(usize, 0), resolved.run_task_action);
    ...
    try std.testing.expectEqual(@as(?usize, 0), state.dialog);
}
...
test "clicking a task row selects it without running its action" {
    ...
    try std.testing.expectEqual(@as(usize, 3), state.selected_task);
    try std.testing.expectEqualStrings("", state.flash);
}
...
test "clicking a tab selects it" {
    ...
    try std.testing.expectEqual(@as(usize, 2), state.selected_tab);
}
...
test "the scope row toggles and the chips flash" {
    ...
    try std.testing.expect(state.scope_open);
    ...
    try std.testing.expect(std.mem.startsWith(u8, state.flash, "new task"));
}
...
test "the scrollbar only appears when the list overflows" {
    ...
    for ([_]struct { h: u16, expect: bool }{
        ...
        var buf = try BufferType.init(gpa, 100, case.h);
        defer buf.deinit();
        view(&state, &buf);
        ...
        try std.testing.expectEqual(case.expect, found);
    }
}
...
test "hit testing is rebuilt every frame rather than accumulated" {
    ...
    view(&state, &buf);
    try std.testing.expectEqual(first, state.hits.len);
}
...
test "narrow and short terminals draw without writing outside the buffer" {
    ...
    for ([_][2]u16{
        ...
        var buf = try BufferType.init(gpa, size[0], size[1]);
        defer buf.deinit();
        view(&state, &buf);
    }
}
...
fn focusFirstTask(state: *State, buf: *BufferType) usize {
    for (0..32) |_| {
...
fn openDialog(state: *State, buf: *BufferType) RectType {
...
test "a click on the dialog's blank interior does not reach the list behind it" {
    ...
    try std.testing.expectEqual(before, state.selected_task);
...
test "a click outside the dialog dismisses it" {
    ...
    try std.testing.expectEqual(@as(?usize, null), state.dialog);
}
...
test "the dialog's buttons beat the dismiss rectangle under them" {
    ...
    try std.testing.expectEqualStrings("Reject", state.flash);
    try std.testing.expectEqual(@as(?usize, null), state.dialog);
}
...
test "the dialog holds the keyboard while it is open" {
    ...
    try std.testing.expectEqual(selected, state.selected_task);
...
test "nothing the dialog draws escapes its frame" {
    ...
    const frame = openDialog(&state, &buf);
    try std.testing.expect(!frame.isEmpty());
    try std.testing.expect(frame.x >= 2);

    var with: [40][100]CellType = undefined;
    for (0..buf.h) |y| for (0..buf.w) |x| {
    ...
    while (y < frame.y + frame.h) : (y += 1) {
        for ([_]u16{ frame.x - 2, frame.x - 1, frame.x + frame.w, frame.x + frame.w + 1 }) |x| {
            ...
            try std.testing.expect(with[y][x].eqlPublic(&without));
            checked += 1;
        }
    }
    // A test that compared nothing would pass too.
    try std.testing.expect(checked >= frame.h * 2);
}
...
test "the dialog takes the keyboard by opening a layer, not by being asked" {
    ...
    var state = demoState();
    view(&state, &buf);
    try std.testing.expect(state.focus.focused().? != .dialog_button);

    _ = openDialog(&state, &buf);
    try std.testing.expectEqual(@as(usize, 0), state.focus.focused().?.dialog_button);
}
...
test "dismissing the dialog returns the keyboard to where it was" {
    ...
    view(&state, &buf);
    try std.testing.expect(state.dialog != null);

    update(&state, .{ .key = .plain(.escape) });
    view(&state, &buf);

    // Not reset to the top of the sidebar. Back to the row the user left.
    try std.testing.expectEqual(before, state.focus.focused().?);
}
...
test "enter runs the focused task, not the one the mouse last touched" {
    ...
    try std.testing.expectEqual(state.selected_task, focused);

    update(&state, .{ .key = .plain(.enter) });
    try std.testing.expectEqual(@as(?usize, focused), state.dialog);
}
...
test "clicking a control moves the keyboard to it" {
    ...
    update(&state, .{ .mouse = .{ .x = point.x, .y = point.y, .kind = .press } });
    view(&state, &buf);
    try std.testing.expectEqual(@as(usize, 1), state.focus.focused().?.task);
}
...
test "focus survives a frame where the focused row scrolled away" {
    ...
    switch (now) {
        .task => |index| try std.testing.expect(index < only_first.len),
        else => {},
    }
...
test "every focusable variant is registered by a drawn frame" {
    ...
    try std.testing.expect(seen_search and seen_tab and seen_scope);
    try std.testing.expect(seen_task and seen_button);
}
...
fn focusSearch(state: *State, buf: *BufferType) void {
    apply(state, .focus_search);
    view(state, buf);
}

fn typeInto(state: *State, buf: *BufferType, text: []const u8) void {
    var it: GraphemeIteratorType = .{ .bytes = text };
...
test "the focused field is greedy with printable keys" {
    ...
    try std.testing.expect(state.focus.has(.search));

    typeInto(&state, &buf, "q1");
    try std.testing.expect(!state.quit);
    try std.testing.expectEqualStrings("q1", state.search.text());
}
...
test "a field that does not hold the keyboard does not eat keys" {
    ...
    try std.testing.expect(state.quit);
    try std.testing.expectEqualStrings("", state.search.text());
}
...
test "the real cursor is placed on the field and nowhere else" {
    ...
    try std.testing.expectEqual(at.y, moved.y);
    try std.testing.expectEqual(at.x + 3, moved.x);

    // Focus elsewhere: the hardware cursor must not stay parked on a field
    // nobody is typing into.
    _ = focusFirstTask(&state, &buf);
    try std.testing.expectEqual(@as(?PositionType, null), state.cursor);
}
...
test "a pasted newline is text, not Enter" {
    ...
    try std.testing.expect(state.focus.has(.search));
    try std.testing.expectEqualStrings("one two", state.search.text());
}
...
test "a paste does not trigger the bindings its characters would" {
    ...
    update(&state, .paste_end);
    try std.testing.expect(!state.quit);
    try std.testing.expectEqualStrings("q", state.search.text());
}
...
test "shift and arrows build a selection the field reports back" {
    ...
    update(&state, .{ .key = .{ .code = .left, .mods = .{ .shift = true } } });
    try std.testing.expectEqualStrings("la", state.search.selected());

    // And typing replaces it.
    typeInto(&state, &buf, "y");
    try std.testing.expectEqualStrings("hoy", state.search.text());
}
...
test "ctrl and arrows move by word" {
    ...
    try std.testing.expectEqualStrings("commit", state.search.text()[state.search.head..]);
}
...
fn mouseAt(x: u16, y: u16, kind: EventType.Mouse.Kind) EventType {
    return .{ .mouse = .{ .x = x, .y = y, .kind = kind } };
}

/// A point over the sidebar's text rather than over any control.
fn inertPoint(state: *const State, buf: *const BufferType) ?PointType {
    var y: u16 = 0;
    while (y < buf.h) : (y += 1) {
        var x: u16 = 0;
        while (x < buf.w) : (x += 1) {
...
test "a drag over inert text selects and copies it" {
    ...
    try std.testing.expect(state.clipboard_len > 0);
    try std.testing.expectEqualStrings("copied", state.flash);
}
...
test "a single click on a control is a click, not a selection" {
    ...
    try std.testing.expectEqual(@as(?RangeType, null), state.selection);
...
test "a double click selects a word even over a control" {
    ...
    try std.testing.expectEqual(GranularityType.word, range.granularity);
    try std.testing.expect(state.clipboard_len > 0);
}
...
test "two slow clicks are two clicks" {
    ...
    const range = state.selection orelse return error.NoSelection;
    try std.testing.expectEqual(GranularityType.character, range.granularity);
}
...
test "a drag that never moved copies nothing" {
    ...
    update(&state, mouseAt(from.x, from.y, .press));
    update(&state, mouseAt(from.x, from.y, .release));
    try std.testing.expectEqual(@as(usize, 0), state.clipboard_len);
}
...
test "what is highlighted is what is copied" {
    ...
    try std.testing.expectEqual(@as(usize, 9), inverted);
...
test "a copy leaves no trailing padding" {
    ...
    const copied = state.clipboard[0..state.clipboard_len];
    try std.testing.expect(copied.len > 0);
    try std.testing.expect(!std.mem.endsWith(u8, copied, " "));
}
