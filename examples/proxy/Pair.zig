...
pub fn certPem(self: *const Pair, buf: []u8) ca.Error![]const u8 {
    return tls.x509.encodePem(buf, tls.x509.cert_label, self.certDer()) catch error.WriteFailed;
}

/// Writes the private key as unencrypted PKCS#8 PEM into `buf`.
pub fn keyPem(self: *const Pair, buf: []u8) ca.Error![]const u8 {
    var der_buf: [256]u8 = undefined;
    const der = tls.x509.encodePrivateKey(&der_buf, self.key_pair) catch return error.WriteFailed;
    return tls.x509.encodePem(buf, tls.x509.key_label, der) catch error.WriteFailed;
}
