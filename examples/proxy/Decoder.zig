...
pub fn init(gpa: std.mem.Allocator) !Decoder {
    ...
    if (source_namespace.c.nghttp2_hd_inflate_new(&self.inflater) != 0) {
...
pub fn deinit(self: *Decoder) void {
    if (self.inflater) |inf| {
        source_namespace.c.nghttp2_hd_inflate_del(inf);
    }
...
fn emit(self: *Decoder, out: *std.Io.Writer) void {
    ...
    while (true) {
        var nv: source_namespace.c.nghttp2_nv = undefined;
        var flags: c_int = 0;
    ...
    _ = source_namespace.c.nghttp2_hd_inflate_end_headers(inf);
