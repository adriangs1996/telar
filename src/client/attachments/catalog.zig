...
pub fn Catalog(comptime Delivery: type) type {
    return struct {
        const Self = @This();
        pub const Slot = struct {
            ...
            pub fn owns(slot: *const Slot, target: Target) bool {
        ...
        pub fn setTarget(store: *Self, target: ?Target) TargetChange {
        ...
        pub fn visibleTarget(store: *const Self) ?Target {
        ...
        pub fn snapshot(store: *const Self) Snapshot {
        ...
        pub fn adopt(store: *Self, capture: *Capture) !void {
        ...
        pub fn remove(store: *Self, id: Id) bool {
        ...
        pub fn removeVisible(store: *Self, target: Target) u8 {
            var removed: u8 = 0;
            for (&store.slots, 0..) |*slot, index| {
        ...
        pub fn planMarkerRemoval(store: *const Self, id: Id, screen: MarkerScreen) ?MarkerRemoval {
            ...
            const removal = switch (slot.marker_policy) {
                .ordered, .stable_number => planPlaceholderRemoval(slot.markerNumber(), ordinal, screen),
                .pasted_path => planPathRemoval(slot.markerPath(), screen),
            } orelse return null;
        ...
        pub fn idAtMarkerDeletion(store: *const Self, screen: MarkerScreen, deletion: MarkerDeletion) ?Id {
            const visible = store.snapshot();
        ...
        pub fn expectMarkerDeletion(store: *Self, target: Target) void {
            for (store.slots) |maybe_slot| {
        ...
        pub fn reconcileMarkers(store: *Self, target: Target, screen: MarkerScreen) u8 {
            const visible = store.snapshot();
        ...
        fn pendingDeletionFor(store: *const Self, target: Target) bool {
            const pending = store.marker_deletion_pending orelse return false;

            return std.meta.eql(pending.target, target);
        }

        fn pathClaimed(store: *const Self, target: Target, uuid: path_marker.Uuid) bool {
            for (store.slots) |maybe_slot| {
                const slot = maybe_slot orelse continue;
        ...
        pub fn openModal(store: *Self, id: Id) bool {
        ...
        pub fn find(store: *Self, id: Id) ?*Slot {
            for (&store.slots) |*maybe_slot| if (maybe_slot.*) |*slot| {
                if (slot.id == id) {
                    return slot;
                }
        ...
        pub fn findConst(store: *const Self, id: Id) ?*const Slot {
            for (&store.slots) |*maybe_slot| if (maybe_slot.*) |*slot| {
                if (slot.id == id) {
                    return slot;
                }
        ...
        pub fn snapshotOrdinal(projection: *const Snapshot, id: Id) ?u8 {
        ...
        pub fn pathForNextUnpaired(store: *const Self, target: Target, buffer: *const ui.Buffer) ?path_marker.Uuid {
            var found: [max_items * 2]path_marker.Marker = undefined;
        ...
        pub fn unpairedPathCount(store: *const Self, target: Target) u8 {
            var count: u8 = 0;
            for (store.slots) |maybe_slot| {
                const slot = maybe_slot orelse continue;
        ...
        pub fn markerForNextUnpaired(store: *const Self, target: Target, buffer: *const ui.Buffer) ?u16 {
        ...
        pub fn unpairedStableCount(store: *const Self, target: Target) u8 {
            var count: u8 = 0;
            for (store.slots) |maybe_slot| {
                const slot = maybe_slot orelse continue;
        ...
        pub fn markerNumberClaimed(store: *const Self, target: Target, number: u16) bool {
            for (store.slots) |maybe_slot| {
                const slot = maybe_slot orelse continue;
