...
const DeliverActivePaneResourcesHandlerType = @import("../../application/panes/DeliverActivePaneResourcesHandler.zig");
...
fn syncAttachmentTarget(raw_context: *anyopaque, target: ?TargetType) ?RectType {
    const client: *Client = @ptrCast(@alignCast(raw_context));
    if (!client.attachment_shelf.syncTarget(target)) {
        return null;
    }
