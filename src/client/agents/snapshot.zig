...
revision: u64 = 0,
items: [source_namespace.max_agents]Agent = undefined,
count: u8 = 0,
...
pub fn replace(snapshot: *Snapshot, input: SnapshotInput) !bool {
    ...
    if (input.agents.len > source_namespace.max_agents) {
...
/// ```
pub fn keyForPane(snapshot: *const Snapshot, location: source_namespace.schema.TabLocation, pane_id: source_namespace.schema.PaneId) ?AgentKey {
    for (snapshot.slice()) |agent| {
