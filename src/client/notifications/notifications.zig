...
pub const default_duration_ns: u64 = default_notification_duration_ms_module *
    std.time.ns_per_ms;
...
test "stored text remains valid utf8 when it hits the byte bound" {
    var center: Center = .{};
    const repeated = "á" ** max_notification_message_bytes_module;
    _ = center.push(0, .{ .title = "UTF-8", .message = repeated });

    const message = center.itemAt(0).?.message();
    try std.testing.expect(message.len <= max_notification_message_bytes_module);
