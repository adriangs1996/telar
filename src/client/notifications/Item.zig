...
level: source_namespace.Level,
target: source_namespace.Target,
title_buffer: [source_namespace.max_title_bytes]u8 = undefined,
title_len: u8,
message_buffer: [source_namespace.max_message_bytes]u8 = undefined,
message_len: u8,
phase: source_namespace.Phase = .entering,
...
pub fn animatedPixels(item: *const Item, full_width: u32) u32 {
    ...
    if (item.transition_position_ns >= source_namespace.transition_duration_ns) {
        return full_width;
    }

    const position: u128 = item.transition_position_ns;
    const duration: u128 = source_namespace.transition_duration_ns;
...
pub fn advanceEntering(item: *Item, now_ns: u64) bool {
    ...
    item.transition_position_ns = @min(
        source_namespace.transition_duration_ns,
...
pub fn nextDeadline(item: *const Item, now_ns: u64, frame_interval_ns: u64) u64 {
    return switch (item.phase) {
        .entering => @min(
            ...
            item.transition_updated_ns +| (source_namespace.transition_duration_ns - item.transition_position_ns),
        ),
