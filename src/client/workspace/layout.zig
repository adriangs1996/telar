...
const Snapshot = @import("LayoutSnapshot.zig");
...
nodes: [source_namespace.max_nodes]Slot = [_]Slot{.{}} ** source_namespace.max_nodes,
root: ?source_namespace.NodeIndex = null,
focused_pane: source_namespace.schema.PaneId = .invalid,
pane_count: u8 = 0,
...
pub fn focused(layout: *const Layout) ?source_namespace.schema.PaneId {
...
/// ```
pub fn clientLayoutNodes(layout: *const Layout, output: *[source_namespace.schema.max_client_layout_nodes]source_namespace.schema.ClientLayoutNode) []const source_namespace.schema.ClientLayoutNode {
    const root = layout.root orelse return output[0..0];
    var stack: [source_namespace.max_nodes]source_namespace.NodeIndex = undefined;
...
/// ```
pub fn fromClientLayout(encoded: source_namespace.schema.ClientTabLayoutView) !Layout {
...
pub fn contains(layout: *const Layout, pane_id: source_namespace.schema.PaneId) bool {
...
/// ```
pub fn displayIndex(layout: *const Layout, pane_id: source_namespace.schema.PaneId) ?u16 {
    var storage: [source_namespace.max_panes]source_namespace.schema.PaneId = undefined;
...
/// ```
pub fn orderedPanes(layout: *const Layout, output: *[source_namespace.max_panes]source_namespace.schema.PaneId) []const source_namespace.schema.PaneId {
    const root = layout.root orelse return output[0..0];
    var stack: [source_namespace.max_nodes]source_namespace.NodeIndex = undefined;
    var stack_len: usize = 1;
    var index: usize = 0;
...
pub fn addRoot(layout: *Layout, pane_id: source_namespace.schema.PaneId) !void {
    if (pane_id == .invalid) {
...
/// ```
pub fn restoreDisplayOrder(layout: *Layout, pane_ids: []const source_namespace.schema.PaneId, focused_pane: source_namespace.schema.PaneId) !void {
    if (pane_ids.len == 0) {
    ...
    for (pane_ids[1..], 1..) |pane_id, index| {
        ...
        restored.setParentRatio(pane_id, source_namespace.equalShareRatio(pane_ids.len - index + 1));
        previous = pane_id;
    }
...
fn setParentRatio(layout: *Layout, pane_id: source_namespace.schema.PaneId, ratio: u16) void {
...
pub fn restoreSaved(layout: *Layout, saved: Layout, panes: PaneSet) bool {
    ...
    for (panes.ids, 0..) |pane_id, index| {
        ...
        if (std.mem.findScalar(source_namespace.schema.PaneId, panes.ids[0..index], pane_id) != null) {
            return false;
        }
    }
...
pub fn splitFocused(layout: *Layout, pane_id: source_namespace.schema.PaneId, axis: source_namespace.Axis) !void {
...
pub fn split(layout: *Layout, request: SplitRequest) !void {
    ...
    if (layout.pane_count == source_namespace.max_panes) {
...
pub fn remove(layout: *Layout, pane_id: source_namespace.schema.PaneId) bool {
...
pub fn focusPane(layout: *Layout, pane_id: source_namespace.schema.PaneId) bool {
...
/// ```
pub fn focusDirection(layout: *Layout, direction: source_namespace.Direction, area: source_namespace.ui.Rect) ?source_namespace.schema.PaneId {
...
fn fullscreenFocusTarget(layout: *const Layout, direction: source_namespace.Direction) ?source_namespace.schema.PaneId {
...
pub fn resizeFocused(layout: *Layout, direction: source_namespace.Direction, area: source_namespace.ui.Rect) bool {
    ...
    const adjusted = switch (direction) {
        .left, .up => previous -| source_namespace.resize_step,
        .right, .down => previous +| source_namespace.resize_step,
    };
    const candidate = std.math.clamp(
        adjusted,
        source_namespace.minimum_split_ratio,
        source_namespace.maximum_split_ratio,
    );
...
/// ```
pub fn canSplit(layout: *const Layout, target: SplitTarget, area: source_namespace.ui.Rect) bool {
    var geometry: Snapshot = .{};
...
/// ```
pub fn prospectiveSplit(layout: *const Layout, target: SplitTarget, area: source_namespace.ui.Rect) ?ProspectiveSplit {
    var geometry: Snapshot = .{};
    layout.snapshot(area, &geometry);
...
pub fn snapshot(layout: *const Layout, area: source_namespace.ui.Rect, output: *Snapshot) void {
...
fn snapshotTiled(layout: *const Layout, area: source_namespace.ui.Rect, output: *Snapshot) void {
    output.reset(.{ .area = area, .revision = layout.revision, .pane_gaps = layout.pane_gaps, .metrics = layout.metrics });
    const root = layout.root orelse return;
    const Pending = struct { node: source_namespace.NodeIndex, area: source_namespace.ui.Rect };
    var stack: [source_namespace.max_nodes]Pending = undefined;
    var stack_len: usize = 1;
...
pub fn views(layout: *const Layout, area: source_namespace.ui.Rect, output: *[source_namespace.max_panes]View) []View {
...
fn resizeSplit(layout: *const Layout, leaf: source_namespace.NodeIndex, direction: source_namespace.Direction) ?source_namespace.NodeIndex {
    const target_axis: source_namespace.Axis = switch (direction) {
        .left, .right => .horizontal,
        .up, .down => .vertical,
    };
    var fallback: ?source_namespace.NodeIndex = null;
    var child = leaf;
...
fn nodeArea(layout: *const Layout, target: source_namespace.NodeIndex, area: source_namespace.ui.Rect) source_namespace.ui.Rect {
    var path: [source_namespace.max_nodes]source_namespace.NodeIndex = undefined;
...
fn ratioFits(layout: *const Layout, branch: Split, candidate: RatioCandidate) bool {
    ...
    const first_extent = source_namespace.extent(first, branch.axis);
    const second_extent = source_namespace.extent(second, branch.axis);
...
fn minimumExtent(layout: *const Layout, node_index: source_namespace.NodeIndex, axis: source_namespace.Axis) u16 {
...
fn allocateNode(layout: *Layout) ?source_namespace.NodeIndex {
...
fn findLeaf(layout: *const Layout, pane_id: source_namespace.schema.PaneId) ?source_namespace.NodeIndex {
...
fn firstLeaf(layout: *const Layout, start: source_namespace.NodeIndex) ?source_namespace.schema.PaneId {
    var current = start;
