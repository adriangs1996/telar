...
const retained = graphics.retained;
const Store = retained.Store;
const pane_id: core.schema.PaneId = @enumFromInt(1);
const image: core.graphics.Image = .{ .key = .{ .image_id = 1, .generation = 1 }, .format = .rgb, .width = 1, .height = 1, .byte_len = 3 };

fn receive(store: *Store, metadata: core.graphics.Image) !void {
...
test "headless catalog holds replaced pixels and credit until the final consumer releases" {
    ...
    const lease = try retained.retain(&store, graphics.identity(pane_id, image.key));
    var next = image;
...
test "rejected shared transfers relinquish their unique name without freeing a borrowed generation" {
    if (!graphics.supportsSharedMemory()) {
        return error.SkipZigTest;
    }

    var source = Store.initSharedMemory(std.testing.allocator);
    defer source.deinit();
    try receive(&source, image);
    const shared = source.images.get(graphics.identity(pane_id, image.key)).?.shared orelse return error.SharedMemoryUnavailable;
    const name = try core.graphics.ShmName.init(shared.slice());
    var store = Store.init(std.testing.allocator);
    defer store.deinit();
    try receive(&store, image);
    const lease = try retained.retain(&store, graphics.identity(pane_id, image.key));
