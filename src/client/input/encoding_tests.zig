...
test "cursor keys follow the focused child's mode" {
    ...
    try std.testing.expectEqualStrings(
        "\x1b[D",
        try encoding_support.encodeKey(&buffer, .plain(.left), .{}),
    );
    try std.testing.expectEqualStrings(
        "\x1bOD",
        try encoding_support.encodeKey(&buffer, .plain(.left), .{ .cursor_keys = true }),
    );
    try std.testing.expectEqualStrings(
        "\x1b[1;5D",
        try encoding_support.encodeKey(&buffer, .{ .code = .left, .mods = .{ .ctrl = true } }, .{ .cursor_keys = true }),
    );
}

test "paste framing follows the focused child's mode" {
    var buffer: [64]u8 = undefined;
    try std.testing.expectEqualStrings("hello", try encoding_support.encodePaste(&buffer, "hello", .{}));
    try std.testing.expectEqualStrings(
        "\x1b[200~hello\x1b[201~",
        try encoding_support.encodePaste(&buffer, "hello", .{ .bracketed_paste = true }),
    );
}
...
test "page keys preserve modifiers" {
    ...
    try std.testing.expectEqualStrings(
        "\x1b[5~",
        try encoding_support.encodeKey(&buffer, .plain(.page_up), .{}),
    );
    try std.testing.expectEqualStrings(
        "\x1b[6;5~",
        try encoding_support.encodeKey(&buffer, .{ .code = .page_down, .mods = .{ .ctrl = true } }, .{}),
    );
}
...
test "Enter modifiers follow the child's keyboard protocol" {
    ...
    for (0..8) |bits| {
        ...
        try std.testing.expectEqualStrings(
            xterm,
            try encoding_support.encodeKey(&buffer, pressed, .{ .modify_other_keys_2 = true }),
        );
        for ([_]u5{ 1, 5, 8, 31 }) |flags| {
            ...
            for ([_]bool{ false, true }) |modify_other_keys| {
                try std.testing.expectEqualStrings(
                    kitty,
                    try encoding_support.encodeKey(&buffer, pressed, .{
...
test "legacy children receive repeats as presses and never receive releases" {
    ...
    try std.testing.expectEqualStrings("\x13", try encoding_support.encodeKey(&buffer, repeat, .{}));
    try std.testing.expectEqualStrings("", try encoding_support.encodeKey(&buffer, release, .{}));

    release.code = .left;
    try std.testing.expectEqualStrings("", try encoding_support.encodeKey(&buffer, release, .{ .cursor_keys = true }));
}
...
test "report-all mode encodes plain key lifecycles" {
    ...
    try std.testing.expectEqualStrings("\x1b[120u", try encoding_support.encodeKey(&buffer, key, modes));
    key.phase = .repeat;
    try std.testing.expectEqualStrings("\x1b[120;1:2u", try encoding_support.encodeKey(&buffer, key, modes));
    key.phase = .release;
    try std.testing.expectEqualStrings("\x1b[120;1:3u", try encoding_support.encodeKey(&buffer, key, modes));
}
...
test "modified Enter encoding reports insufficient output space" {
    ...
    try std.testing.expectError(error.WriteFailed, encoding_support.encodeKey(&xterm_buffer, pressed, .{ .modify_other_keys_2 = true }));
}
