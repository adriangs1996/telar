const Handler = @This();
const model_module = @import("../../root.zig").model;
const source_namespace = @import("history_browser.zig");
model: *model_module.Model,

pub const Page = model_module.HistoryPageResult;
...
pub fn requestPage(handler: Handler, id: u64, scope: source_namespace.schema.HistoryScope) bool {
...
pub fn output(handler: Handler, reply: source_namespace.schema.HistoryOutput) bool {
    return handler.model.history_palette.applyOutput(reply);
}

pub fn fail(handler: Handler, reply: source_namespace.schema.RequestFailed) bool {
