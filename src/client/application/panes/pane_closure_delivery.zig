...
const PaneClosureDeliveryTestingModel = @import("PaneClosureDeliveryTestingModel.zig");
const PaneClosureDeliveryEffectsCapture = @import("PaneClosureDeliveryEffectsCapture.zig");
const DeliverPaneClosureHandler = @import("DeliverPaneClosureHandler.zig");
const std = @import("std");

pub const Event = union(enum) {
    ignore_attachment: PaneIdType,
    complete_close: PaneIdType,
...
fn deliveryHandler(testing: *PaneClosureDeliveryTestingModel, capture: *PaneClosureDeliveryEffectsCapture) DeliverPaneClosureHandler {
    return .{
        .model = testing.model,
...
test "DeliverPaneClosureHandler releases an active exit before focus and geometry repair" {
    var testing = try PaneClosureDeliveryTestingModel.init();
    ...
    var capture: PaneClosureDeliveryEffectsCapture = .{
        .model = testing.model,
        .exit = exit,
...
test "DeliverPaneClosureHandler limits inactive and stale exits to idempotent cleanup" {
    var inactive_testing = try PaneClosureDeliveryTestingModel.init();
    defer inactive_testing.deinit();
    const inactive_exit = inactive_testing.model.retirePane(inactive_testing.inactive_pane);
    var inactive_capture: PaneClosureDeliveryEffectsCapture = .{
    ...
    var stale_testing = try PaneClosureDeliveryTestingModel.init();
    defer stale_testing.deinit();
    const missing: PaneIdType = @enumFromInt(99);
    const stale_exit = stale_testing.model.retirePane(missing);
    var stale_capture: PaneClosureDeliveryEffectsCapture = .{ .model = stale_testing.model, .exit = stale_exit };
...
test "DeliverPaneClosureHandler skips geometry after the active tab becomes empty" {
    ...
    var capture: PaneClosureDeliveryEffectsCapture = .{ .model = testing.model, .exit = exit };
    var handler = deliveryHandler(&testing, &capture);
...
test "DeliverPaneClosureHandler preserves cleanup across active delivery failures" {
    var sync_testing = try PaneClosureDeliveryTestingModel.init();
    defer sync_testing.deinit();
    const sync_exit = sync_testing.model.retirePane(sync_testing.second);
    var sync_capture: PaneClosureDeliveryEffectsCapture = .{
    ...
    var resize_testing = try PaneClosureDeliveryTestingModel.init();
    defer resize_testing.deinit();
    const resize_exit = resize_testing.model.retirePane(resize_testing.second);
    var resize_capture: PaneClosureDeliveryEffectsCapture = .{
