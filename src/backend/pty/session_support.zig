...
test "executable lookup uses PATH from the explicit child environment" {
    ...
    var executable = try temp.dir.createFile(io, "telar-path-command", .{ .permissions = std.Io.File.Permissions.fromMode(0o700) });
...
test "spawn rejects an executable without execute permission" {
    ...
    var file = try temp.dir.createFile(io, "not-executable", .{ .permissions = std.Io.File.Permissions.fromMode(0o600) });
...
test "spawn preserves execvp shell fallback semantics" {
    ...
    var script = try temp.dir.createFile(io, "script", .{ .permissions = std.Io.File.Permissions.fromMode(0o700) });
...
test "wait reaps a real child once and shutdown after the reap is harmless" {
    ...
    try std.testing.expectEqual(exit_mod.Exit{ .exited = 3 }, exit);
...
test "foreground inspection identifies the session leader" {
    ...
    session.shutdown();
    try std.testing.expectEqual(exit_mod.Exit{ .signaled = .KILL }, try session.wait());
}
