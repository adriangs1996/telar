...
const InputObservation = @import("ObserverInputObservation.zig");
const OutputObservation = @import("ObserverOutputObservation.zig");
...
pub fn init(observer: *Observer, initialization: Initialization) !void {
    ...
    try observer.stream.handler.resize(source_namespace.vtResize(size));
...
pub fn queueResize(observer: *Observer, size: source_namespace.schema.TerminalSize) void {
...
pub fn processSealed(observer: *Observer, processing: Processing, sink: anytype) void {
    ...
    for (batch.events[0..batch.event_count]) |event| switch (event) {
        ...
        .resize => |size| observer.stream.handler.resize(source_namespace.vtResize(size)) catch {
            observer.failures +|= 1;
            stats.failed = true;
        },
    ...
    const signal = if (processing.provider == .codex or
        ...
        source_namespace.mergeSignals(observer.manifests, phrase_signal, prompt_scan.scanReadyPrompt(&observer.terminal));
    if (signal) |candidate| {
...
fn publishSignal(observer: *Observer, signal: ?agent_detection.Signal, now_ms: ?i64) ?agent_detection.Signal {
    ...
    if (observer.last_signal) |previous| {
        if (std.meta.eql(previous, current) and !(current.provider == .codex and current.ready_confirmed)) {
            ...
            if (clock - observer.last_signal_ms < source_namespace.signal_refresh_ms) {
                return null;
            }
        }
    }
...
fn pushControl(observer: *Observer, event: source_namespace.Event) void {
    var batch = &observer.batches[observer.active];
...
fn resetState(observer: *Observer, cwd: []const u8, size: source_namespace.schema.TerminalSize) !void {
    observer.tracker.deinit(&observer.terminal);
    observer.stream.deinit();
