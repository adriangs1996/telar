...
pub fn init(gpa: std.mem.Allocator) !Channel {
    const request_storage = try gpa.alloc(model.Request, channel_support.request_capacity);
    errdefer gpa.free(request_storage);

    const response_storage = try gpa.alloc(model.Response, channel_support.response_capacity);
