...
pub fn init(configured: []const []const u8) !Policy {
    ...
    std.mem.sort([]const u8, policy.exact_storage[0..policy.exact_count], {}, interception_policy.lessThan);
    std.mem.sort([]const u8, policy.suffix_storage[0..policy.suffix_count], {}, interception_policy.suffixLessThan);
    policy.exact_count = interception_policy.deduplicate(policy.exact_storage[0..policy.exact_count], interception_policy.compare);
    policy.suffix_count = interception_policy.deduplicate(policy.suffix_storage[0..policy.suffix_count], interception_policy.compareSuffix);
    return policy;
}
