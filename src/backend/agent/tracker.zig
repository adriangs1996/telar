...
repository: source_namespace.Repository = .{},
restored_titles: source_namespace.RestoredTitles = .{},
...
/// ```
pub fn observeSessionReference(tracker: *Tracker, identity: source_namespace.Identity, reference: types.SessionReference) bool {
...
/// ```
pub fn projectedProvider(tracker: *const Tracker, key: source_namespace.PaneKey) source_namespace.schema.AgentProvider {
...
/// ```
pub fn sessionReference(tracker: *const Tracker, key: source_namespace.PaneKey) ?types.SessionReference {
...
/// ```
pub fn durableTitle(tracker: *const Tracker, key: source_namespace.PaneKey) ?source_namespace.SessionTitle {
    const agent = tracker.repository.findConst(key) orelse return null;
...
/// ```
pub fn restoreTitle(tracker: *Tracker, key: source_namespace.PaneKey, title: source_namespace.SessionTitle) bool {
...
/// }
/// ```
pub fn acknowledge(tracker: *Tracker, key: source_namespace.PaneKey, now_ms: i64) source_namespace.AcknowledgeResult {
...
/// });
/// ```
pub fn observeProcess(tracker: *Tracker, observation: source_namespace.ProcessObservation) bool {
...
/// ```
pub fn clearProcess(tracker: *Tracker, key: source_namespace.PaneKey) bool {
...
/// }
/// ```
pub fn observeProxy(tracker: *Tracker, observation: source_namespace.ProxyObservation) bool {
...
/// });
/// ```
pub fn observeScreen(tracker: *Tracker, observation: source_namespace.ScreenObservation) bool {
...
/// ```
pub fn remove(tracker: *Tracker, key: source_namespace.PaneKey) bool {
...
/// ```
pub fn snapshot(tracker: *const Tracker, entries: *[source_namespace.max_records]source_namespace.schema.AgentSnapshotEntry) []const source_namespace.schema.AgentSnapshotEntry {
    var count: usize = 0;
...
/// ```
pub fn projectedStatus(tracker: *const Tracker, key: source_namespace.PaneKey) ?source_namespace.schema.AgentStatus {
    const agent = tracker.repository.findConst(key) orelse return null;
...
/// ```
pub fn observeInput(tracker: *Tracker, key: source_namespace.PaneKey, bytes: []const u8) bool {
    const agent = tracker.repository.find(key) orelse return false;
...
/// ```
pub fn finishDescription(tracker: *Tracker, result: *const description.Result) ?source_namespace.DescriptionFinished {
...
/// ```
pub fn setManualTitle(tracker: *Tracker, key: source_namespace.PaneKey, value: []const u8) !bool {
    const agent = tracker.repository.find(key) orelse return false;
...
/// ```
pub fn reportTitle(tracker: *Tracker, identity: source_namespace.Identity, value: []const u8) !bool {
    const agent = tracker.ensure(identity) orelse return false;
...
fn resolveProxyAgent(tracker: *Tracker, observation: *const source_namespace.ProxyObservation) ?*Agent {
...
fn ensure(tracker: *Tracker, identity: source_namespace.Identity) ?*Agent {
...
fn removeStored(tracker: *Tracker, key: source_namespace.PaneKey) bool {
