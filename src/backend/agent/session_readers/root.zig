...
pub fn probe(job: Job) Completion {
    ...
    switch (job.watch.kind) {
        .claude_transcript => @import("claude.zig").probe(job, &completion),
        .codex_state => @import("codex.zig").probe(job, &completion),
    }
...
test "codex state probe reads the thread name, a NULL name as empty and nothing for unknown threads" {
    ...
    var unnamed = watch;
    unnamed.session = try @import("../types.zig").SessionReference.init("unnamed", 1);
    ...
    var unknown = watch;
    unknown.session = try @import("../types.zig").SessionReference.init("nope", 1);
