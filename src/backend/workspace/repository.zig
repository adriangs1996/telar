...
pub fn reserveGitProbe(repository: *Repository, request: ProbeSchedule) ?git.Probe {
    ...
    var stalest: ?*source_namespace.Workspace = null;
...
pub fn cancelGitProbe(repository: *Repository, workspace: source_namespace.schema.WorkspaceId) void {
...
pub fn init(state: *source_namespace.State, gpa: std.mem.Allocator) Repository {
...
pub fn find(repository: *Repository, location: source_namespace.schema.WorkspaceLocation) ?*source_namespace.Workspace {
...
/// });
/// ```
pub fn insert(repository: *Repository, request: Insert) !source_namespace.schema.TabLocation {
...
pub fn propose(repository: *Repository, request: Insert) !Proposal {
    ...
    const workspace_id = try source_namespace.schema.id.workspace(repository.state.next_workspace_id);
    const tab_id = try source_namespace.schema.id.tab(repository.state.next_tab_id);
    const path = try repository.gpa.dupe(u8, request.path);
    errdefer repository.gpa.free(path);
    const workspace = try source_namespace.Workspace.init(.{
        .id = workspace_id,
        .path = path,
...
pub const Restore = struct {
    ...
    first_tab_id: source_namespace.schema.TabId,
...
/// ```
pub fn restoreWorkspace(repository: *Repository, request: Restore) !source_namespace.schema.TabLocation {
    if (repository.state.count == repository.state.items.len) {
        return error.WorkspaceLimitReached;
    }
    ...
    const path = try repository.gpa.dupe(u8, request.path);
    errdefer repository.gpa.free(path);
    var workspace = try source_namespace.Workspace.init(.{
        .id = request.id,
        .path = path,
    ...
    repository.state.next_workspace_id = @max(repository.state.next_workspace_id, source_namespace.schema.id.raw(request.id) + 1);
    repository.state.next_tab_id = @max(repository.state.next_tab_id, source_namespace.schema.id.raw(request.first_tab_id) + 1);
...
/// ```
pub fn restoreTab(repository: *Repository, location: source_namespace.schema.TabLocation, label: []const u8) !void {
    ...
    repository.state.next_tab_id = @max(repository.state.next_tab_id, source_namespace.schema.id.raw(location.tab_id) + 1);
    state_mod.advanceRevision(repository.state);
}
...
/// ```
pub fn remove(repository: *Repository, workspace_id: source_namespace.schema.WorkspaceId) bool {
    for (&repository.state.items) |*slot| {
        const workspace = if (slot.*) |*value| value else continue;
...
pub fn nextTabId(repository: *const Repository) !source_namespace.schema.TabId {
    return source_namespace.schema.id.tab(repository.state.next_tab_id);
}
...
/// ```
pub fn recordTabCreated(repository: *Repository, tab_id: source_namespace.schema.TabId) void {
    std.debug.assert(source_namespace.schema.id.raw(tab_id) == repository.state.next_tab_id);
