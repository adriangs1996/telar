...
const Spec = @import("SessionSpec.zig");
...
reader_storage: source_namespace.Io.File.MultiReader.Buffer(1) = undefined,
reader: source_namespace.Io.File.MultiReader = undefined,
stderr_future: ?source_namespace.Io.Future(anyerror!void) = null,
...
/// ```
pub fn open(io: source_namespace.Io, gpa: std.mem.Allocator, options: OpenOptions) !*Session {
...
pub fn exchange(session: *Session, spec: Spec, request: Request) !*effects.Result {
    ...
    const timeout: source_namespace.Io.Timeout = .{ .deadline = .fromNow(session.io, .{
        .clock = .awake,
...
fn readExact(session: *Session, output: []u8, timeout: source_namespace.Io.Timeout) !void {
