...
const source_namespace = @import("transfer_preparation.zig");
...
pub fn request(queue: *Queue, value: Request) bool {
    ...
    for (queue.entries, 0..) |entry, index| {
        if (entry) |existing| {
            if (source_namespace.same(existing.request, value)) {
                return false;
            }
        } else if (free == null) {
            free = index;
...
pub fn take(queue: *Queue, value: Request) !?Frozen {
    for (&queue.entries) |*slot| {
        ...
        if (!source_namespace.same(entry.request, value)) {
            continue;
        }
...
pub fn process(queue: *Queue, storage: anytype, media: *allocation.PaneMediaAllocator) void {
    for (&queue.entries) |*slot| {
        ...
        const image = storage.imageById(entry.request.key.image_id) orelse {
            source_namespace.discardEntry(slot, media);
            continue;
        };
        ...
        entry.result = source_namespace.freeze(entry.request, pixels, media);
    }
}
