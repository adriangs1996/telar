...
pub fn Type(comptime Application: type) type {
    return struct {
        ...
        const agent_description_runtime_port: source_namespace.agent_description_coordinator.RuntimePort(Application) = .{
            .start = startAgentDescription,
            .persist = persistAgentDescription,
            .pump_clients = pumpRuntimeClients,
        };

        const RuntimeAgentDescriptionCoordinator = source_namespace.agent_description_coordinator.Coordinator(Application, agent_description_runtime_port);
        ...
        const agent_maintenance_runtime_port: source_namespace.agent_maintenance_coordinator.RuntimePort(Application) = .{
            .rearm_tick = rearmAgentMaintenance,
            .now_ms = runtimeWallClockMs,
            .pump_clients = pumpRuntimeClients,
        };

        const RuntimeAgentMaintenanceCoordinator = source_namespace.agent_maintenance_coordinator.Coordinator(Application, agent_maintenance_runtime_port);
        ...
        fn runtimeWallClockMs(application: *Application) i64 {
            return source_namespace.Io.Timestamp.now(application.io, .real).toMilliseconds();
        }

        const proxy_observation_runtime_port: source_namespace.proxy_observation_adapter.RuntimePort(Application) = .{
            .rearm_receive = rearmProxyObservation,
            .schedule_description = scheduleDescription,
            .pump_clients = pumpRuntimeClients,
        };

        const RuntimeProxyObservationAdapter = source_namespace.proxy_observation_adapter.Adapter(Application, proxy_observation_runtime_port);
        ...
        const proxy_capture_runtime_port: source_namespace.proxy_capture_adapter.RuntimePort(Application) = .{
            .rearm_receive = rearmProxyCapture,
            .now_ms = runtimeWallClockMs,
        };

        const RuntimeProxyCaptureAdapter = source_namespace.proxy_capture_adapter.Adapter(Application, proxy_capture_runtime_port);
        ...
        const plugin_effects_runtime_port: source_namespace.plugin_effects_adapter.RuntimePort(Application) = .{
        ...
        const RuntimePluginEffectsAdapter = source_namespace.plugin_effects_adapter.Adapter(Application, plugin_effects_runtime_port);
