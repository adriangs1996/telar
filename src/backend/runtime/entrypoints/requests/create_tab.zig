...
const CreateTabController = @import("CreateTabController.zig");
...
fn testingRequest(workspace: WorkspaceLocationType, request_id: RequestIdType) !CreateTabViewType {
    return .{
...
test "Controller maps create-tab input and queues the canonical result" {
    ...
    const root_pane_id = try pane_module(17);
    var responses: ResponseQueue = .{};
    var stub: StubCreateTab = .{ .result = .{
        .created = try TabCreatedType.init(canonical_location, 1, "canonical"),
        .root_pane_id = root_pane_id,
    } };
    var controller = CreateTabController.init(&responses, stub.executor());
