const Capture = @This();
const source_namespace = @import("send_coordinator.zig");
...
pub fn resolve(capture: *Capture, client: u8) ?*FakeSession {
...
pub fn releaseSend(capture: *Capture, session: *FakeSession) void {
    capture.record(.release_send);
    session.send_pending = false;
}

pub fn isClosing(capture: *Capture, session: *FakeSession) bool {
...
pub fn completeDelivery(capture: *Capture, _: *FakeSession, result: anyerror!void) FakeCompletion {
...
pub fn detachAfterSend(capture: *Capture, _: *FakeSession, pane: u8) void {
    capture.record(.detach_after_send);
    capture.detached_pane = pane;
}

pub fn shouldCloseAfterReply(capture: *Capture, session: *FakeSession) bool {
...
pub fn pumpClient(capture: *Capture, _: *FakeSession) !void {
