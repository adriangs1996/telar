    ...
    return struct {
        ...
        pub fn route(router: Self, message: ClientMessageType) !void {
            return switch (message) {
                ...
                .agent_prompt => |request| handlers.agent_prompt(router.context, request),
                .agent_interrupt => |request| handlers.agent_interrupt(router.context, request),
                .agent_resume => |request| handlers.agent_resume(router.context, request),
                .agent_approval => |request| handlers.agent_approval(router.context, request),
                .query_agent_thread => |request| handlers.query_agent_thread(router.context, request),
                .query_agent_history => |request| handlers.query_agent_history(router.context, request),
