...
pub const SlotIndex = pane_mod.SlotIndex;
pub const max_panes = pane_mod.max_panes;
...
pub const PaneDetached = @import("PaneDetached.zig");
...
pub const SelectionQuery = @import("SelectionQuery.zig");

pub const AttachmentStore = @import("AttachmentStore.zig");
...
pub fn encodeNextGraphics(attachment: *Attachment, preparation: Attachment.GraphicsPreparation) !?[]const u8 {
...
pub fn stageNextTransfer(attachment: *Attachment, global_credit: usize) !StageResult {
    ...
    while (image_iterator.next()) |entry| {
        ...
        while (placement_iterator.next()) |placement_entry| {
            ...
            if (index == core.graphics.max_placements_per_pane) {
                break;
            }
...
test "a shared-transport attachment ships one name instead of pixel chunks" {
    if (comptime !graphics.shared_memory_supported) {
