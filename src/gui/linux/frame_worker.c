...
static void *run(void *context) {
    ...
    for (;;) {
        while (!self->pending && !self->stopping) pthread_cond_wait(&self->condition, &self->mutex);
...
telar_frame_worker *telar_frame_worker_create(telar_renderer *renderer) {
    ...
    if (telar_gui_pipe(self->wake) != 0) { free(self); return NULL; }
    if (pthread_mutex_init(&self->mutex, NULL) != 0) goto fail_pipe;
    if (pthread_cond_init(&self->condition, NULL) != 0) goto fail_mutex;
    if (pthread_create(&self->thread, NULL, run, self) != 0) goto fail_condition;
    return self;
fail_condition:
...
void telar_frame_worker_submit(telar_frame_worker *self, telar_gui_viewport viewport, const telar_gui_frame *frame) {
    pthread_mutex_lock(&self->mutex);
    assert(!self->pending && !self->done);
