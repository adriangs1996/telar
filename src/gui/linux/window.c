...
static void refresh_window_title(window *self) {
  if (self->toplevel == NULL || self->callbacks.window_title == NULL) {
    return;
  }

  telar_gui_window_title update = {0};
  if (self->callbacks.window_title(self->context, &update) <= 0) {
    return;
  }
  ...
  update.bytes[update.len] = '\0';
  xdg_toplevel_set_title(self->toplevel, (const char *)update.bytes);
}
...
static void registry_global(void *data, struct wl_registry *registry,
                            ...
                            uint32_t version) {
  window *self = data;
  const telar_registry_global global = {.registry = registry,
                                        .name = name,
                                        .version = version,
                                        .interface = interface};
...
static void draw(window *self) {
  if (!self->configured || self->in_flight || !self->dirty) {
    return;
  }
...
static void surface_configure(void *data, struct xdg_surface *surface,
  ...
  if (self->renderer == NULL) {
    telar_gui_viewport viewport = {self->width, self->height, 1.0f};
    self->renderer =
        telar_renderer_create(self->display, self->surface, viewport);
    if (self->renderer == NULL) {
      self->failed = true;
      self->closing = true;
      return;
...
int telar_gui_run(const char *title, void *context,
  ...
  if (self.compositor == NULL || self.shell == NULL) {
    fprintf(stderr,
            "telar gui: the compositor lacks wl_compositor or xdg_wm_base\n");
    destroy(&self);
    return -1;
  }
  ...
  while (!self.closing) {
    ...
    struct pollfd fds[] = {
        {wl_display_get_fd(self.display),
         POLLIN | (write_blocked ? POLLOUT : 0), 0},
    ...
    if (self.worker != NULL && (fds[3].revents & POLLIN)) {
      ...
      if (telar_frame_worker_take(self.worker, &token, &outcome)) {
        ...
        if (outcome == TELAR_RENDER_FAILED) {
          self.failed = true;
          break;
        }
        if (outcome == TELAR_RENDER_RETRY) {
          telar_frame_clock_cancel(&self.clock);
          self.dirty = true;
        }
      }
