...
@interface TelarView : NSView <NSWindowDelegate, NSTextInputClient>
@end
...
- (instancetype)initWithFrame:(NSRect)frame context:(void *)render_context callbacks:(const telar_gui_callbacks *)callback_table {
...
- (void)updateLayer { [self requestDraw]; }
...
- (void)startWakeSource {
    wake_source = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, callbacks.wake_fd, 0, dispatch_get_main_queue());
    __weak TelarView *weak = self;
    dispatch_source_set_event_handler(wake_source, ^{
        TelarView *view = weak;
        if (view == nil || view->closed) return;
        telar_gui_drain(view->callbacks.wake_fd);
        int result = view->callbacks.pump(view->context);
        if (result < 0) [view.window close];
        else if (result > 0) [view requestDraw];
    });
...
- (void)draw {
    ...
    if (drawable == nil) {
        ...
        if (self.window.occlusionState & NSWindowOcclusionStateVisible) [self requestDraw];
        return;
    }
    ...
    [commands addCompletedHandler:^(id<MTLCommandBuffer> completed) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            int result = view->callbacks.pump(view->context);
            if (result < 0) [view.window close];
            else if (view->dirty || result > 0) [view requestDraw];
        });
    }];
    [commands commit];
}
...
- (void)sendText:(NSString *)text kind:(uint32_t)kind {
    ...
    telar_gui_input event = {.kind = kind, .phase = kind == 1 ? text_phase : 1, .text = data.bytes, .len = data.length};
    if (!callbacks.input(context, event)) NSBeep();
}

- (void)paste:(id)sender {
    NSString *text = [NSPasteboard.generalPasteboard stringForType:NSPasteboardTypeString];
    if (text != nil) [self sendText:text kind:2];
}

- (void)keyDown:(NSEvent *)event {
    text_phase = event.isARepeat ? 2 : 1;
    if ((event.modifierFlags & NSEventModifierFlagCommand) && [[event.charactersIgnoringModifiers lowercaseString] isEqualToString:@"v"]) {
        [self paste:nil];
        return;
    }
    ...
    switch (event.keyCode) {
        case 36: case 76: code = 1; break;
        case 48: code = 2; break;
        case 51: code = 3; break;
        case 53: code = 4; break;
        case 126: code = 5; break;
        case 125: code = 6; break;
        case 123: code = 7; break;
        case 124: code = 8; break;
        case 115: code = 9; break;
        case 119: code = 10; break;
        case 117: code = 11; break;
        case 116: code = 12; break;
        case 121: code = 13; break;
    }
    ...
    if (code != 0) {
        telar_gui_input input = {.kind = 3, .code = code, .mods = mods, .phase = event.isARepeat ? 2 : 1};
        if (!callbacks.input(context, input)) NSBeep();
    } else if (mods & 4) {
        NSString *characters = event.charactersIgnoringModifiers;
        if (characters.length == 1) {
            telar_gui_input input = {.kind = 4, .code = [characters characterAtIndex:0], .mods = mods, .phase = event.isARepeat ? 2 : 1};
            if (!callbacks.input(context, input)) NSBeep();
        }
    } else {
...
- (void)setMarkedText:(id)value selectedRange:(NSRange)selection replacementRange:(NSRange)replacement {
    NSAttributedString *text = [value isKindOfClass:NSAttributedString.class] ? value : [[NSAttributedString alloc] initWithString:value];
    [marked setAttributedString:text];
}
- (void)unmarkText { [marked deleteCharactersInRange:NSMakeRange(0, marked.length)]; }
- (BOOL)hasMarkedText { return marked.length != 0; }
- (NSRange)markedRange { return marked.length ? NSMakeRange(0, marked.length) : NSMakeRange(NSNotFound, 0); }
...
- (NSRect)firstRectForCharacterRange:(NSRange)range actualRange:(NSRangePointer)actual {
    return [self.window convertRectToScreen:[self convertRect:NSMakeRect(0, 0, 1, 16) toView:nil]];
}
- (void)doCommandBySelector:(SEL)selector {}

- (void)windowWillClose:(NSNotification *)notification {
    closed = YES;
    if (wake_source != nil) dispatch_source_cancel(wake_source);
    if (active_commands != nil) [active_commands waitUntilCompleted];
    [NSApp stop:nil];
...
int telar_gui_run(const char *title, void *context, const telar_gui_callbacks *callbacks) {
    @autoreleasepool {
        ...
        TelarView *view = [[TelarView alloc] initWithFrame:window.contentView.bounds context:context callbacks:callbacks];
...
int telar_gui_clipboard(const uint8_t *bytes, size_t len) {
    ...
    return [NSPasteboard.generalPasteboard setString:text forType:NSPasteboardTypeString] ? 0 : -1;
}
