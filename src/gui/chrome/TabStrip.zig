...
pub fn paint(strip: TabStrip) !void {
    ...
    try canvas.fillPixels(strip.bands.shoulder, palette.panel_bg);
    const area = strip.bands.tab_strip;
    if (area.width <= 0 or area.height <= 0) {
        return;
    }

    try canvas.fillPixels(area, palette.panel_bg);
...
fn tab(strip: TabStrip, entry: TabEntry) !void {
    ...
    if (active) {
        ...
        try canvas.fillRoundedPixels(.{ .x = bounds.x, .y = bounds.y, .width = bounds.width, .height = radius * 2 }, .{ .radius = radius, .color = .default });
        try canvas.fillPixels(.{ .x = bounds.x, .y = bounds.y + radius, .width = bounds.width, .height = @max(0, bounds.height - radius) }, .default);
    } else if (hovered) {
        try canvas.fillRoundedPixels(.{ .x = bounds.x, .y = bounds.y, .width = bounds.width, .height = @max(0, bounds.height - chrome.px(4)) }, .{ .radius = chrome.px(6), .color = palette.surface1 });
    }
