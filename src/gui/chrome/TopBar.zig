...
pub fn paint(bar: TopBar) !void {
    ...
    if (projection.proxy_tls_active or projection.proxy_system_trusted) {
        ...
        right -= badge_width;
        _ = try canvas.textPixels(.{ .x = right, .y = row.y, .width = badge_width, .height = row.height }, .{
            .text = badge,
    ...
    const slot: SlotRow = .{ .context = bar.context };
    const slot_width = @min(slot.width(&projection.bar_state.layout.top_right), @max(0, right - left));
    if (slot_width > 0) {
        right -= slot_width;
        try slot.paint(.{ .x = right, .y = row.y, .width = slot_width, .height = row.height }, &projection.bar_state.layout.top_right);
    }
