...
//! across the window, the sidebar band down the left, the tab strip over the
//! workbench with its shoulder over the sidebar, and the status bar along
...
/// Places the bands for one canvas. The tab strip starts where the grid
/// does, after the sidebar band and its gap or after the window padding, and
/// runs to the window edge, so trailing pixels stay chrome. The sidebar band
/// runs from under the tab strip to the status bar.
...
pub fn resolve(canvas: *const Canvas) Bands {
    ...
    const split = @min(width, @as(f32, @floatFromInt(canvas.origin[0])));
    const body_top = @min(height, top + strip);
    return .{
...
pub fn contains(bands: Bands, x: f64, y: f64) bool {
    return within(bands.top_bar, x, y) or within(bands.shoulder, x, y) or within(bands.tab_strip, x, y) or within(bands.sidebar, x, y) or within(bands.status_bar, x, y);
}
