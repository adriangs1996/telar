...
pub fn draw(widget: PickerModal, canvas: *Canvas) !void {
    ...
    if (results.len == 0) {
        try canvas.text(rows.row(0), .{ .text = "No matches", .color = palette.subtext0 });
    }

    if (content.h > 2) {
        try canvas.text(content.row(content.h - 1), .{ .text = "Up/Down select  Enter open  Esc cancel", .color = palette.subtext0 });
    }
}
