...
pub fn draw(widget: Transcript, source: *Canvas) !void {
    ...
    if (canvas.widgets) |state| {
        _ = try state.dispatcher.add(flow.navigation(widget.target(&canvas, flow.scroll_limit)));
    }
    if (flow.len == 0) {
    ...
    if ((widget.thread.history != null or widget.thread.transcript.?.truncated) and (widget.thread.transcript_scroll >= flow.scroll_limit or widget.thread.history != null and widget.thread.transcript_scroll == 0)) {
        var error_text: [320]u8 = undefined;
        const label: []const u8 = if (widget.thread.history) |window| if (window.failed) @import("std").fmt.bufPrint(&error_text, "{s} · scroll again to retry", .{window.failureMessage()}) catch window.failureMessage() else if (window.pending != null) "Loading messages…" else if (widget.thread.transcript_scroll == 0) "Scroll down to continue" else if (window.has(.older)) "Scroll up for earlier messages" else "Beginning of conversation" else "Scroll up for earlier messages";
...
fn register(widget: Transcript, canvas: *Canvas, limit: u32) !void {
    if (canvas.widgets) |state| {
        _ = try state.dispatcher.add(widget.target(canvas, limit));
    }
}

fn target(widget: Transcript, canvas: *Canvas, limit: u32) @import("interaction/Target.zig") {
...
fn drawRequest(widget: Transcript, canvas: *Canvas, request: *const core.AgentApprovalRequest) !void {
    ...
    const limit: u32 = @intFromFloat(@ceil(maximum / canvas.chrome.px(24)));
    try widget.register(canvas, limit);
    text.bounds.y -= @max(0, maximum - @as(f32, @floatFromInt(@min(widget.thread.transcript_scroll, limit))) * canvas.chrome.px(24));
