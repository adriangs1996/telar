...
pub const HistoryStatsTopIterator = struct {
    ...
    pub fn next(iterator: *HistoryStatsTopIterator) !?HistoryStatsTop {
        ...
        const command = try iterator.decoder.readSized16();
        if (command.len == 0 or command.len > max_history_command_bytes)
            return error.InvalidByteString;
...
fn decodeHistoryStatsQuery(decoder: *wire.Decoder) !HistoryStatsQuery {
    const request_id = try id.request(try decoder.readInt(u64));
    const scope = try decodeHistoryScope(try decoder.readByte());
    var scope_value: []const u8 = "";
    var pane_id: PaneId = .invalid;
...
fn decodeHistoryStats(decoder: *wire.Decoder) !HistoryStatsView {
    const request_id = try id.request(try decoder.readInt(u64));
    ...
    for (0..top_count) |_| {
        ...
        if ((try decoder.readSized16()).len > max_history_command_bytes)
            return error.InvalidByteString;
    }
    return .{
        .request_id = request_id,
...
pub fn decodeClient(payload: []const u8) !ClientMessage {
    ...
    const message: ClientMessage = switch (tag) {
        ...
        .request_pane_focus => .{ .request_pane_focus = try decodeRequestPaneFocus(&decoder) },
        .complete_pane_focus => .{ .complete_pane_focus = try decodeCompletePaneFocus(&decoder) },
    };
...
pub fn decodeServer(payload: []const u8) !ServerMessage {
    ...
    const message: ServerMessage = switch (tag) {
        ...
        .pane_focus_result => .{ .pane_focus_result = try decodePaneFocusResult(&decoder) },
    };
...
fn decodeRequestPaneFocus(decoder: *wire.Decoder) !RequestPaneFocus {
...
fn decodeCompletePaneFocus(decoder: *wire.Decoder) !CompletePaneFocus {
...
fn decodePaneFocusCommand(decoder: *wire.Decoder) !PaneFocusCommand {
...
fn decodePaneFocusResult(decoder: *wire.Decoder) !PaneFocusResult {
