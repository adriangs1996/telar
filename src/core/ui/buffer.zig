...
cells: []source_namespace.Cell,
w: u16,
h: u16,
gpa: std.mem.Allocator,

/// Nothing is written outside this. Starts as the whole buffer.
clip: source_namespace.Rect,
stack: [max_clip_depth]source_namespace.Rect = undefined,
depth: u8 = 0,
...
pub fn init(gpa: std.mem.Allocator, w: u16, h: u16) !Buffer {
    const cells = try gpa.alloc(source_namespace.Cell, @as(usize, w) * @as(usize, h));
    @memset(cells, .{});
    return .{
        .cells = cells,
        .w = w,
...
pub fn pushClip(b: *Buffer, r: source_namespace.Rect) void {
...
pub fn resize(b: *Buffer, w: u16, h: u16) !void {
    ...
    const cells = try b.gpa.alloc(source_namespace.Cell, @as(usize, w) * @as(usize, h));
    b.gpa.free(b.cells);
    b.cells = cells;
    b.w = w;
    b.h = h;
...
pub fn area(b: *const Buffer) source_namespace.Rect {
    return .{ .w = b.w, .h = b.h };
}

pub fn at(b: *Buffer, x: u16, y: u16) ?*source_namespace.Cell {
    if (x >= b.w or y >= b.h) {
        return null;
    }
...
pub fn clear(b: *Buffer, style: source_namespace.Style) void {
    @memset(b.cells, .{ .style = style });
}

pub fn fill(b: *Buffer, r: source_namespace.Rect, fill_value: Fill) void {
...
/// ```
pub fn setCell(b: *Buffer, point: source_namespace.Point, value: CellWrite) void {
    ...
    var len: u8 = @intCast(@min(text.len, source_namespace.Cell.max_bytes));
...
/// ```
pub fn writeText(b: *Buffer, r: source_namespace.Rect, write: TextWrite) u16 {
    ...
    var it: source_namespace.GraphemeIterator = .{ .bytes = write.text };
...
/// ```
pub fn writeTruncated(b: *Buffer, r: source_namespace.Rect, truncated: TruncatedText) u16 {
    const write: TextWrite = .{ .point = truncated.point, .text = truncated.text, .style = truncated.style };

    if (truncated.max_width == 0) {
        return 0;
    }
    if (source_namespace.measure(write.text) <= truncated.max_width) {
    ...
    const marker = source_namespace.measure(ellipsis);
...
/// ```
pub fn writeLeftTruncated(b: *Buffer, r: source_namespace.Rect, truncated: TruncatedText) u16 {
    const write: TextWrite = .{ .point = truncated.point, .text = truncated.text, .style = truncated.style };

    if (truncated.max_width == 0) {
        return 0;
    }
    const total = source_namespace.measure(write.text);
...
/// ```
pub fn writeRight(b: *Buffer, r: source_namespace.Rect, write: RightAlignedText) u16 {
    const width = source_namespace.measure(write.text);
    if (width > r.w) {
...
/// ```
pub fn box(b: *Buffer, r: source_namespace.Rect, box_value: Box) void {
    if (r.w < 2 or r.h < 2) {
        return;
    }
    ...
    if (box_value.title) |title| {
        const inside: source_namespace.Rect = .{ .x = r.x + 2, .y = r.y, .w = r.w -| 4, .h = 1 };
...
/// ```
pub fn fillWithoutCorners(b: *Buffer, r: source_namespace.Rect, style: source_namespace.Style) void {
    if (r.w < 2 or r.h < 2) {
        return;
    }
