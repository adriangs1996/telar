...
pub const Surface = struct {
    ...
    pub fn validate(surface: Surface) !void {
        ...
        const byte_count = std.math.mul(usize, pixel_count, 4) catch
            return error.SurfaceTooLarge;
        if (surface.pixels.len != byte_count) return error.InvalidSurface;
    }
};
...
pub const Rasterizer = struct {
    ...
    pub fn setPixelHeight(rasterizer: *Rasterizer, pixel_height: u16) !void {
        if (pixel_height == 0) return error.InvalidPixelHeight;
        if (rasterizer.pixel_height == pixel_height) return;
    ...
    pub fn drawText(
        ...
        if (rasterizer.pixel_height == 0) return error.FontSizeNotSet;
        ...
        for (shaped.glyphs, shaped.positions) |glyph, position| {
            if (glyph.codepoint == 0) return error.MissingGlyph;
            const next_x = pen_x + position.x_advance;
            if (fixed26_6Round(next_x - origin_fixed) > max_width) break;
    ...
    fn shapeText(rasterizer: *Rasterizer, text: []const u8) !ShapedText {
        ...
        if (position_count != glyph_count) return error.ShapingFailed;
        return .{
...
fn blendBitmap(
    ...
    if (bitmap.pixel_mode != ft.FT_PIXEL_MODE_GRAY and
        bitmap.pixel_mode != ft.FT_PIXEL_MODE_MONO) return error.UnsupportedPixelMode;
    ...
    for (0..rows) |source_y| {
        ...
        if (target_y < 0 or target_y >= surface.height) continue;
        ...
        for (0..columns) |source_x| {
            ...
            if (target_x < 0 or target_x >= surface.width) continue;
