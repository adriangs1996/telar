...
pub fn write(self: *KittyGraphicsWriter, writer: *source_namespace.Io.Writer) source_namespace.Io.Writer.Error!usize {
    ...
    var compress_budget: usize = source_namespace.compression_slice_per_frame;
    ...
    if (self.store.delivery.partial) |partial| {
        ...
        if (!alive) {
            ...
            written += try source_namespace.writeTransmissionAbort(writer);
            written += try source_namespace.writeDeleteImage(writer, partial.external_id);
            ...
            const progress = try source_namespace.writeTransmissionChunks(writer, .{
    ...
    if (self.store.delivery.delete_overflow) {
        written += try source_namespace.writeDeleteImageRange(writer, 1, 0x3fffffff);
        delivery.recoverDeleteOverflow(self.store);
    }
    while (delivery.popDelete(self.store)) |deletion| written += switch (deletion) {
        .image => |image_id| try source_namespace.writeDeleteImage(writer, image_id),
        .placement => |placement| try source_namespace.writeDeletePlacement(
            writer,
            placement.image_id,
    ...
    while (images.next()) |entry| {
        ...
        if (image.shared) |*shared| {
            if (!image.delivery.force_direct and self.store.shared_memory) {
                const emitted = try source_namespace.writeSharedTransmission(writer, .{
    ...
    while (placements.next()) |entry| {
        ...
        const image = self.store.images.get(source_namespace.identity(
...
fn writeFallbackPlacements(self: *KittyGraphicsWriter, writer: *source_namespace.Io.Writer, frame: FallbackFrame) source_namespace.Io.Writer.Error!usize {
    if (!self.store.paneVisible(frame.partial.key.pane_id)) {
        return 0;
    }
    var written: usize = 0;
    for (frame.partial.fallbacks[0..frame.partial.fallback_count]) |fallback| {
        const key: source_namespace.PlacementIdentity = .{
...
fn geometry(self: *const KittyGraphicsWriter, geometry_input: PlacementGeometry) ?source_namespace.OutputPlacement {
    ...
    const width, const height = source_namespace.destinationSize(.{
    ...
    const destination: source_namespace.graphics.Rect = .{
    ...
    const bounds: source_namespace.graphics.Rect = .{
    ...
    const clipped = source_namespace.graphics.clipScaled(destination, .{
        .x = source.x,
        .y = source.y,
        .width = source_width,
