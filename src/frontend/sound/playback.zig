...
active: bool = false,
queued: ?source_namespace.Kind = null,
...
/// ```
pub fn init(configuration: source_namespace.Config) Playback {
...
/// ```
pub fn request(playback: *Playback, kind: source_namespace.Kind) source_namespace.RequestOutcome {
    if (!playback.configuration.allows(kind)) {
        return .ignored;
    }

    if (playback.active) {
        playback.queued = source_namespace.coalesce(playback.queued, kind);
        return .queued;
    }
...
/// ```
pub fn complete(playback: *Playback) ?source_namespace.Kind {
...
/// ```
pub fn configure(playback: *Playback, configuration: source_namespace.Config) void {
