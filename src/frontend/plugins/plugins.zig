...
pub fn executeWorker(io: std.Io, gpa: std.mem.Allocator, request: WorkerRequest) !WorkerResult {
    ...
    try std.Io.Dir.cwd().createDir(io, snapshot_root, std.Io.File.Permissions.fromMode(0o700));
    defer std.Io.Dir.cwd().deleteTree(io, snapshot_root) catch {};
...
pub fn loadPackage(context: LoadContext, configured_path: []const u8) !Package {
    ...
    const root_len = try std.Io.Dir.cwd().realPathFile(io, joined, &root_buffer);
    ...
    var entry_buffer: [std.fs.max_path_bytes]u8 = undefined;
    const entry_len = try std.Io.Dir.cwd().realPathFile(io, candidate, &entry_buffer);
    const entry = entry_buffer[0..entry_len];
    if (!pathInside(root, entry)) {
        return error.EntrypointEscapesPackage;
    }
    const entry_bytes = try std.Io.Dir.cwd().readFileAlloc(
        io,
        entry,
        gpa,
...
pub fn inspectPackage(gpa: std.mem.Allocator, io: std.Io, path: []const u8) !Package {
...
pub fn installPackage(gpa: std.mem.Allocator, io: std.Io, installation: Installation) !void {
    const package = installation.package;
    const destination = installation.destination;

    if (std.Io.Dir.cwd().statFile(io, destination, .{ .follow_symlinks = false })) |stat| {
    ...
    try std.Io.Dir.cwd().createDir(io, temp, std.Io.File.Permissions.fromMode(0o700));
    var committed = false;
    defer if (!committed) std.Io.Dir.cwd().deleteTree(io, temp) catch {};

    var source = try std.Io.Dir.cwd().openDir(io, package.root(), .{ .iterate = true });
    ...
    while (try walker.next(io)) |entry| {
        ...
        switch (entry.kind) {
            .directory => try std.Io.Dir.cwd().createDir(
                io,
                target,
                std.Io.File.Permissions.fromMode(0o700),
            ),
            .file => try std.Io.Dir.copyFile(
                source,
                entry.path,
                std.Io.Dir.cwd(),
                target,
    ...
    try std.Io.Dir.cwd().rename(temp, std.Io.Dir.cwd(), destination, io);
    committed = true;
}
...
pub fn updatePackageFingerprint(gpa: std.mem.Allocator, io: std.Io, update: FingerprintUpdate) void {
    const hasher = update.hasher;
    const root = update.root;

    hasher.update(root);
    var directory = std.Io.Dir.cwd().openDir(io, root, .{ .iterate = true }) catch {
