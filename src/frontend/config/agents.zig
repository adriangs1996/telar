...
pub fn parse(state: *lua_api.c.lua_State, runtime: *RuntimeSnapshotType, diagnostic: *DiagnosticType) !void {
    ...
    for (1..count + 1) |position| {
        ...
        const entry = lua_api.c.lua_absindex(state, -1);
        ...
        _ = lua_api.c.lua_getfield(state, entry, "name");
...
fn parseCommandTools(state: *lua_api.c.lua_State, input: EntryInput, diagnostic: *DiagnosticType) !void {
    ...
    const tools = lua_api.c.lua_absindex(state, -1);
    const count = lua_api.c.lua_rawlen(state, tools);
    ...
    for (1..count + 1) |tool_position| {
        _ = lua_api.c.lua_rawgeti(state, tools, @intCast(tool_position));
        defer value.pop(state, 1);
        if (lua_api.c.lua_type(state, -1) != lua_api.c.LUA_TTABLE) {
