...
const InputDecisionType = @import("telar-client").InputDecision;
...
const SourceType = @import("telar-client").Source;
const min_command_timeout_ms_module = @import("telar-client").min_command_timeout_ms;
const max_command_timeout_ms_module = @import("telar-client").max_command_timeout_ms;
...
const ClientInputCallbackRefCallbackRef = @import("telar-client").InputCallbackRef;
...
fn parseSidebar(generation: *Generation, index: c_int, diagnostic: *DiagnosticType) !void {
    ...
    if (lua_api.c.lua_type(state, -1) != lua_api.c.LUA_TNIL) {
        ...
        generation.snapshot.sidebar_rendering = capabilities.SidebarRendering.parse(value) catch {
