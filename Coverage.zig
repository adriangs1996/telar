...
pub fn excludeCSourceCoverage(coverage: Coverage, b: *std.Build, module: *std.Build.Module) void {
    ...
    for (module.link_objects.items) |link_object| switch (link_object) {
        .c_source_file => |source| source.flags = source_namespace.cFlags(b, source.flags, true),
        .c_source_files => |sources| sources.flags = source_namespace.cFlags(b, sources.flags, true),
        else => {},
    };
}
